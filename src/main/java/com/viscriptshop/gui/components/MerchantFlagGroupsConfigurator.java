...
public class MerchantFlagGroupsConfigurator extends ConfiguratorGroup {
    ...
    public MerchantFlagGroupsConfigurator(MerchantInfo merchant) {
        ...
        setCanCollapse(true);
        setTips("viscript_shop.data.merchant.flagGroups.tip");
        configuratorContainer.layout(layout -> {
            layout.gapAll(3);
        });
        rebuild();
    }
    ...
    private void rebuild() {
        ...
        if (groups == null) {
            ...
            merchant.setFlagGroups(groups);
        }
        if (merchant.getFlagGroupMode() == null) {
            merchant.setFlagGroupMode(MerchantFlagGroup.GroupMatchMode.OR);
        }

        addConfigurator(new SelectorConfigurator<>(
                "viscript_shop.data.flag_group.group_mode",
                merchant::getFlagGroupMode,
                mode -> {
                    merchant.setFlagGroupMode(mode);
                    notifyChanges();
                },
        ...
        if (groups.isEmpty()) {
            ...
            empty.addInlineChild(new Label()
                    .setText("viscript_shop.editor.flag_group.empty")
        ...
        actions.addInlineChild(new Button()
                ...
                .setOnClick(event -> {
                    merchant.getFlagGroups().add(new MerchantFlagGroup());
                    notifyChanges();
                    rebuild();
                })
                .layout(layout -> layout.widthPercent(100)));
    ...
    private UIElement createFlagChip(MerchantFlagGroup group, String flag) {
        ...
        remove.buttonStyle(style -> style.baseTexture(IGuiTexture.EMPTY).hoverTexture(IGuiTexture.EMPTY).pressedTexture(IGuiTexture.EMPTY));
